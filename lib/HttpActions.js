...
class HttpActions {
    ...
    _post(request, response, url) {
        ...
        const is_local_execution = me.auth.isValidLocalAccessToken(accessToken);
        const user = me.auth.getuserForAccessToken(accessToken);
        if (user === null) {
        ...
        for (let i = 0; i < reqdata.inputs.length; i++) {
            ...
            switch (intent) {
                ...
                case 'action.devices.DISCONNECT':
                    ...
                    me.auth.removeAllTokensForUser(user);
    ...
    reportState(deviceId, states, notifications) {
        ...
        if (!this.auth._jwtkey.private_key) {
            return;
        }

        const auth = new google.auth.GoogleAuth({
            keyFile: this._jwtKeyFile.startsWith(path.sep) ? this._jwtKeyFile : path.join(this._userDir, this._jwtKeyFile),
