...
class HttpActions {
    ...
    _post(request, response, url) {
        ...
        if (user === null) {
            me._smarthome.error('HttpActions:httpActionsRegister(/' + url + '): no accessToken found');
    ...
    _sync(requestId, response) {
        ...
        let devices = this._smarthome.getProperties();
        if (!devices) {
    ...
    _query(requestId, devices, response) {
        ...
        let deviceIds = this._smarthome.getDeviceIds(devices);

        let states = this._smarthome.getStates(deviceIds);
        if (!states) {
            response.status(500).set({
                'Access-Control-Allow-Headers': 'Content-Type, Authorization',
            }).json({ error: 'failed' });
    ...
    _execDevice(command, device, is_local) {
        ...
        const cur_device = me._smarthome.getDevice(deviceId);
        ...
        if (reportState) {
            ...
            process.nextTick(() => {
                let s = me._smarthome.getStates([deviceId]);
    ...
    _reachable_devices(requestId, devices, response) {
        ...
        let reachableDevices = this._smarthome.getReachableDeviceIds();
        if (!reachableDevices) {
            response.status(500).set({
                'Access-Control-Allow-Headers': 'Content-Type, Authorization',
            }).json({ error: 'failed' });
    ...
    reportState(deviceId, states, notifications) {
        ...
        this._smarthome.debug('HttpActions:reportState(): deviceId = ' + deviceId);
        if (states) {
            this._smarthome.debug('HttpActions:reportState(): states = ' + JSON.stringify(states));
        }
        if (notifications) {
            this._smarthome.debug('HttpActions:reportState(): notifications = ' + JSON.stringify(notifications));
        }

        if (!this._smarthome.IsHttpServerRunning()) {
            me._smarthome.error('HttpActions:reportState(): http server is not running');
            return;
        }

        if (!this._smarthome.auth.isUserLoggedIn()) {
            me._smarthome.error('HttpActions:reportState(): no user logged!');
            return;
        }
        ...
        this._smarthome.debug('HttpActions:reportState(): postData = ' + JSON.stringify(postData));
        ...
        homegraph.devices.reportStateAndNotification({ 'requestBody': postData })
            .then(result => {
                me._smarthome.debug('HttpActions:reportState(): success');
            })
            .catch(error => {
                let myError = {
    ...
    requestSync() {
        ...
        if (!this._smarthome.IsHttpServerRunning()) {
            me._smarthome.error('HttpActions:requestSync(): http server is not running');
            return;
        }

        if (!this._smarthome.auth.isUserLoggedIn()) {
            me._smarthome.error('HttpActions:requestSync(): no user logged!');
            return;
        }
