...
class HttpActions {
            ...
            for (let i = 0; i < reqdata.inputs.length; i++) {
                ...
                switch (intent) {
                    ...
                    case 'action.devices.IDENTIFY':
                        me.debug('HttpActions:httpActionsRegister(/smarthome): IDENTIFY');
                    ...
                    case 'action.devices.REACHABLE_DEVICES':
                        me.debug('HttpActions:httpActionsRegister(/smarthome): REACHABLE_DEVICES');
    ...
    _reachable_devices(requestId, devices, response) {
        ...
        this.debug('HttpActions:_reachable_devices(): reachableDevices = ' + JSON.stringify(reachableDevices));

        response.status(200).json(reachableDevices);
