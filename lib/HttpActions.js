...
const isLocalIP = function(ipString) {
    const ip = Addr(ipString) 
    return LOOPBACK_NETWORKS.some((network) => network.contains(ip)) || PRIVATE_NETWORKS.some((network) => network.contains(ip))
}
...
class HttpActions {
    ...
    httpActionsRegister(httpRoot, appHttp) {
         ...
         * }
         */
        appHttp.post(me.Path_join(httpRoot, 'local_smarthome'), function(request, response) {
            if (!isLocalIP(request.ip)) {
                response.status(200).set({
                    'Access-Control-Allow-Headers': 'Content-Type, Authorization',
                }).json({});
            }
            _post(request, response, 'local_smarthome');
        });

        appHttp.post(me.Path_join(httpRoot, 'smarthome'), function(request, response) {
            _post(request, response, 'smarthome');
        });
        ...
        appHttp.options(me.Path_join(httpRoot, 'local_smarthome'), function(request, response) {
            _options(request, response);
        });
    }
    /******************************************************************************************************************
    ...
    _post(request, response, url) {
        ...
        if (!res) {
            me._mgmtNode.error('HttpActions:httpActionsRegister(/' + url + '): missing authorization header; res = ' + JSON.stringify(res));
        ...
        if (res.length != 2 || res[0] != 'Bearer') {
            me._mgmtNode.error('HttpActions:httpActionsRegister(/' + url + '): invalid authorization data; res = ' + JSON.stringify(res));
        ...
        if (!me.isValidAccessToken(accessToken)) {
            me._mgmtNode.error('HttpActions:httpActionsRegister(/' + url + '): no accessToken found');
        ...
        if (!reqdata.inputs) {
            me._mgmtNode.error('HttpActions:httpActionsRegister(/' + url + '): missing reqdata.inputs');
        ...
        for (let i = 0; i < reqdata.inputs.length; i++) {
            ...
            if (!intent) {
                me._mgmtNode.error('HttpActions:httpActionsRegister(/' + url + '): missing intent');
            ...
            switch (intent) {
                ...
                case 'action.devices.EXECUTE':
                     ...
                     * }
                     */
                    me._exec(reqdata.requestId, reqdata.inputs[i].payload.commands, response);
                    break;
                ...
                default:
                    me._mgmtNode.error('HttpActions:httpActionsRegister(/' + url + '): invalid intent');
    ...
    _reachable_devices(requestId, devices, response) {
        ...
        this.debug('HttpActions:_reachable_devices(): reachableDevices = ' + JSON.stringify(reresBodychableDevices));
