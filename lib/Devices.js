...
class Devices {
    ...
    SetState(clientId, states, replaceAll) {
        ...
        // update states
        if (replaceAll) {
            Object.keys(me._devices[clientId].states).forEach(function(key) { 
                delete me._devices[clientId].states[key]; 
            });
    ...
    GetIdFromName(name) {
        let nodeId;
        if (name) {
            Object.keys(this._devices).forEach(nid => {
                let elm = this._devices[nid];
                if (elm.properties.name.name === name) {
                    nodeId = nid;
                }
            });
        }
    ...
    execCommand(device, command, is_local) {
        let me = this;
        ...
        if (typeof this._nodes[device.id].execCommand === 'function') {
            let result = this._nodes[device.id].execCommand(device, command, this._devices[device.id], is_local);
    ...
    //
    //
    execDevice(device, doUpdate, params, original_params, is_local) {
        let me = this;

        if (!this._devices[device.id]) {
            me.debug('Device:execDevice(): device does not exist: ' + JSON.stringify(device));
            return false;
        }
        ...
        if (doUpdate) {
            this._nodes[device.id].updated(this._devices[device.id], params, original_params, is_local);
        }
