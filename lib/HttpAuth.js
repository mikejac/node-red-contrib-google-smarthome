    ...
    httpAuthRegister(httpRoot, appHttp) {
        ...
        appHttp.post(path.join(httpRoot, 'oauth'), function(req, res) {
            ...
            if (response_type !== 'code') {
                me._mgmtNode.error('HttpAuth:httpAuthRegister(POST /oauth): response_type ' + response_type + ' must equal "code"');
                return res.status(500)
                    .send('response_type ' + response_type + ' must equal "code"');
            }

            if (!me.isValidClient(req.body.client_id)) {
                me._mgmtNode.error('HttpAuth:httpAuthRegister(POST /oauth): client_id ' + req.body.client_id + ' invalid');
