...
class HttpAuth {
    ...
    httpAuthRegister(httpRoot, appHttp) {
        ...
        appHttp.get(path.join(httpRoot, 'oauth'), function(req, res) {
            ...
            fs.readFile(path.join(__dirname, 'frontend/login.html'), 'utf8', function (err, data) {
                ...
                res.send(data.replace(/GOOGLE_CLIENT_ID/g, googleClientId).replace(/USE_GOOGLE_LOGIN/g, '' + useGoogleClientAuth));
            });
        });
        //
        //
            ...
            if (me.useGoogleClientAuth()) {
                ...
                if (req.body.id_token) {
                    ...
                    const ticket = client.verifyIdToken({
                        ...
                        me.handleUserAuth(req, res, email, '', isValidUser);
                    })
                    .catch(function(err) {
                        me.debug('HttpAuth:httpAuthRegister(POST /oauth): verifyIdToken error ' + err);
                        me.handleUserAuth(req, res, 'google', '', false);
                    });
                } else {
                    me.handleUserAuth(req, res, 'google', '', false);
                }
                ...
                me.handleUserAuth(req, res, req.body.username, req.body.password, isValidUser);
            }
        });
     ...
     * }
     */
    handleUserAuth(req, res, username, password, isValidUser) {
        if (!isValidUser) {
            process.nextTick(() => {
                this.emitter.emit('/oauth', 'invalid user', username, password);
            });

            this.debug('HttpAuth:handleUserAuth(): invalid user');
        ...
        let authCode = this.generateAuthCode();

        if (authCode) {
            process.nextTick(() => {
                this.emitter.emit('/oauh', 'authCode successful', username, password);
            });
            ...
            this.debug('HttpAuth:handleUserAuth(): authCode failed');
            return res.redirect(util.format(
