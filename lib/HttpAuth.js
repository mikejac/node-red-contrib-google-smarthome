...
class HttpAuth {
    ...
    httpAuthRegister(httpRoot, appHttp) {
        ...
        appHttp.get(me.Path_join(httpRoot, 'oauth'), function(req, res) {
            me.debug('HttpAuth:httpAuthRegister(GET /oauth) query ' + JSON.stringify(req.query));

            if (req.query.response_type !== 'code') {
                me.error('HttpAuth:httpAuthRegister(GET /oauth): response_type ' + req.query.response_type + ' must equal "code"');
                return res.status(500)
                    .send('response_type ' + req.query.response_type + ' must equal "code"');
            }

            if (!me.auth.isValidClient(req.query.client_id)) {
                me.error('HttpAuth:httpAuthRegister(GET /oauth): client_id ' + req.query.client_id + ' invalid');
                return res.status(500).send('client_id ' + req.query.client_id + ' invalid');
            }

            const useGoogleClientAuth = me.auth.useGoogleClientAuth();
            const googleClientId = useGoogleClientAuth ? me.auth.getGoogleClientId() : '';
        ...
        appHttp.post(me.Path_join(httpRoot, 'oauth'), function(req, res) {
            ...
            const my_uri = req.protocol + '://' + req.get('Host') + me.Path_join(httpRoot, 'oauth');
            ...
            if (!me.auth.isValidClient(client_id)) {
                me.error('HttpAuth:httpAuthRegister(POST /oauth): client_id ' + client_id + ' invalid');
                return res.status(500).send('client_id ' + client_id + ' invalid');
            }

            if (!me.auth.isValidRedirectUri(req.body.redirect_uri || '', me._debug ? my_uri : '')) {
                me.error('HttpAuth:httpAuthRegister(POST /oauth): redirect_uri ' + req.body.redirect_uri + ' invalid');
                return res.status(500).send('redirect_uri ' + req.body.redirect_uri + ' invalid');
            }

            if (me.auth.useGoogleClientAuth()) {
                me.debug('HttpAuth:httpAuthRegister(POST /oauth): Google login');
                if (req.body.id_token) {
                    const {OAuth2Client} = require('google-auth-library');
                    const googleClientId = me.auth.getGoogleClientId();
                    ...
                    client.verifyIdToken({
                        ...
                        const isValidUser = me.auth.isGoogleClientEmailValid(email);
                    ...
                    .catch(function(err) {
                        me.error('HttpAuth:httpAuthRegister(POST /oauth): verifyIdToken error ' + err);
                ...
                let isValidUser = me.auth.isValidUser(req.body.username, req.body.password);
        ...
        appHttp.all(me.Path_join(httpRoot, 'token'), function(req, res) {
            ...
            if (!me.auth.isValidClient(clientId, clientSecret)) {
                me.error('HttpAuth:httpAuthRegister(/token): invalid client id or secret');
                return res.status(400).send('invalid client id or secret');
            }

            if ('authorization_code' == grantType) {
                return me.handleAuthCode(req, res, me._debug ? my_uri : '');
            } else if ('refresh_token' == grantType) {
                return me.handleRefreshToken(req, res);
            } else {
                me.error('HttpAuth:httpAuthRegister(/token): grant_type ' + grantType + ' is not supported');
    ...
    handleUserAuth(req, res, username, password, isValidUser, httpRoot) {
        ...
        let authCode = this.auth.generateAuthCode(username);
    ...
    handleAuthCode(req, res, my_uri) {
        ...
        try {
            let token = this.auth.exchangeAuthCode(code, redirect_uri, my_uri);
    ...
    handleRefreshToken(req, res) {
        ...
        try {
            let token = this.auth.refreshAccessToken(refreshToken);
