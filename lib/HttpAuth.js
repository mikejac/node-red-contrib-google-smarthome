...
class HttpAuth {
    ...
    handleAuthCode(req, res) {
        ...
        try {
            let token = this.exchangeAuthCode(code);
    ...
    handleRefreshToken(req, res) {
        ...
        if (!this.isValidRefreshToken(refreshToken)) {
            this.debug('HttpAuth:handleRefreshToken(): refreshToken ' + refreshToken + ' invalid');
            return res.status(500).send('refreshToken ' + refreshToken + ' invalid');
        }

        let auth = this.getAuth();
        let duration = this.getAccessTokenDuration();
        res.status(200).json({
            token_type: 'bearer',
