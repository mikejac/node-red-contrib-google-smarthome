...
class GoogleSmartHome extends Aggregation(Auth, Devices, HttpAuth, HttpActions) {
    ...
    UnregisterUrl(REDapp) {
        ...
        if (this._httpNodeRoot !== false && REDapp._router) {
            ...
            var post_urls = [me.Path_join(me._httpPath, 'oauth'), me.Path_join(me._httpPath, 'smarthome'), me.Path_join(me._httpLocalPath, 'smarthome_local')];
            var options_urls = [me.Path_join(me._httpPath, 'smarthome'), me.Path_join(me._httpLocalPath, 'smarthome_local')];
        ...
        try {
            ...
            if (this._httpNodeRoot !== false) {
                ...
                REDapp._router.stack.forEach((r) => {
                    if (r.route && r.route.path && (r.route.path.startsWith(this._httpPath) || r.route.path.startsWith(_httpLocalPath))) {
                        me.debug('SmartHome:Start(): url ' + r.route.path + " registred for " + me.GetRouteType(r));
                    }
                });

                this._localExecutionPort = REDserver.address().port

                this.dnssdAd = dnssd.Advertisement(dnssd.tcp('nodered-google'), this._localExecutionPort, {txt: {clientId: this._nodeId}})
                this.dnssdAd.start();
