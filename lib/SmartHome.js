...
class GoogleSmartHome extends Aggregation(Auth, Devices, HttpAuth, HttpActions) {
    constructor(mgmtNode, nodeId, userDir, httpNodeRoot, useGoogleLogin, googleClientId, emails, username, password, accessTokenDuration, usehttpnoderoot,
        ...
        if (nodeRedUsesHttps && httpLocalPort)
        {
            this.debug("Node-RED is using HTTPS but not local http port was defined, local execution will fail.")
        }
    ...
    StartMDNSAdvertisement(port) {
        ...
        this.dnssdAd.on('error', function (err) {
            me._mgmtNode.error('SmartHome:Start(): dnssd-Ad: err:' + err);
    ...
    Start(REDapp, REDserver) {
        try {
            ...
            if (this._httpLocalPort > 0) {
                me.debug('SmartHome:Start(listen): starting local fullfillment');
                ...
                this.localHttpServer.on('error', function (err) {
                    me._mgmtNode.error('SmartHome:Start(): local err:' + err);
