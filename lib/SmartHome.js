...
class GoogleSmartHome {
    ...
    Start(REDapp, REDserver) {
        ...
        try {
            ...
            this.localUDPServer = udp.createSocket('udp4');

            this.localUDPServer.on('error',function(error){
                console.log('Server UDP Error: ' + error);
                this.localUDPServer.close();
                this.localUDPServer = null;
            });
            
            this.localUDPServer.on('message',function(msg,info){
                console.log('Data received from client : ' + msg.toString());
                console.log('Received %d bytes from %s:%d\n',msg.length, info.address, info.port);
              
                // TODO sending msg
                const devices_state = me.devices.getProperties();
                const sync_res = JSON.stringify({
            ...
            this.localUDPServer.on('listening',function(){
            ...
            this.localUDPServer.on('close',function(){
                console.log('Socket is closed !');
            });
              
            this.localUDPServer.bind(8882);
