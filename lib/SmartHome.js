...
class GoogleSmartHome extends Aggregation(Auth, Devices, HttpAuth, HttpActions) {
    ...
    Start(REDapp, REDserver) {
        try {
            ...
            if (this._httpsPort > 0) {
                ...
                this.dnssdAd = dnssd.Advertisement(dnssd.tcp('nodered-google'), this._httpsPort, {txt: {clientId: this._nodeId}})
                this.dnssdAd.start();
                ...
                if (this._httpNodeRoot !== false) {
                    ...
                    this.dnssdAd = dnssd.Advertisement(dnssd.tcp('nodered-google'), REDserver.address().port, {txt: {clientId: this._nodeId}})
                    this.dnssdAd.start();
                    this._dnssdAdRunning = true;
    ...
    getCustomData() {
        return {
            ...
            httpSSLOffload: this._sslOffload,
            httpPathPrefix: this.Path_join(this._httpPath, ""),
