        ...
        if (this._httpNodeRoot !== false && REDapp._router) {
            ...
            REDapp._router.stack.forEach(function (route, i/*, routes*/) {
                if (route.route && (
                    ...
                    me._mgmtNode.debug('SmartHome:Stop(): removing url: ' + route.route.path + " registred for " + me.GetRouteType(route));
                    to_remove.unshift(i);
                    // routes.splice(i, 1);
                }
            });
            ...
            REDapp._router.stack.forEach(function (route) {
                if (route.route) me._mgmtNode.debug('SmartHome:Stop(): remaining url: ' + route.route.path + " registred for " + me.GetRouteType(route));
            });
        }
    }
    //
