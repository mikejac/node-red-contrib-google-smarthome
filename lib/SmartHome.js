    ...
    Start(REDapp, REDserver) {
        try {
            ...
            if (this._httpsPort > 0) {
                ...
                if (mdns) {
                    this.mdnsAd = mdns.createAdvertisement(mdns.tcp('node-red-google-smarthome', this._httpsPort, {txtRecord: {clientId: this._nodeId}}))
                    this.mdnsAd.start();
                    this._mdnsAdRunning = true;

                    this.mdnsAd.on('error', function (err) {
                        me._mgmtNode.error('SmartHome:Start(): mdnsAd: err:' + err);

                        process.nextTick(() => {
                            me.emitter.emit('mdnsad', 'error', err);
                        });
                    });
                }
            } else {
                if (this._httpNodeRoot !== false) {
                    ...
                    if (mdns) {
                        this.mdnsAd = mdns.createAdvertisement(mdns.tcp('node-red-google-smarthome', REDserver.address().port, {txtRecord: {clientId: this._nodeId}}))
                        this.mdnsAd.start();
                        this._mdnsAdRunning = true;
