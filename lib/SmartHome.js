...
class GoogleSmartHome {
    constructor(mgmtNode, nodeId, userDir, httpNodeRoot, useGoogleLogin, googleClientId, emails, username, password, accessTokenDuration, usehttpnoderoot,
        ...
        this.deviceScanType         = "UDP";
        this.deviceScanPort         = 8882; // TODO CAN BE httpLocalPort??
        this.deviceScanPacket       = "Ciao Claudio da GOOGLE";
        this.localUDPServer         = null;
    ...
    StopUDPDeviceScanServer() {
        if (this.localUDPServer !== null) {
            this.localUDPServer.close();
            this.localUDPServer = null;
        }
    }
    //
    ...
    StartUDPDeviceScanServer() {
        const me = this;
        me.debug('Starting sevice Scan UDP server on port ' + me.deviceScanPort);

        me.StopUDPDeviceScanServer();

        me.localUDPServer = udp.createSocket('udp4');

        me.localUDPServer.on('error',function(error){
            me.debug('Server UDP Error: ' + error);
            me.StopUDPDeviceScanServer();
        });
        
        me.localUDPServer.on('message',function(msg,info){
            const data = msg.toString();
            me.debug('Sevice Scan UDP server data received : ' + data);
            me.debug('Sevice Scan UDP server received %d bytes from %s:%d\n', msg.length, info.address, info.port);
          
            if (data === me.deviceScanPacket) {
                const sync_res = Buffer.from(JSON.stringify({clientId: this._nodeId}), 'utf8');

                me.localUDPServer.send(sync_res, info.port, info.address, function(error){
                    if(error){
                ...
                me.debug("Sevice Scan UDP server error wront message received!");
            }
        });

        me.localUDPServer.on('listening',function(){
            const address = me.localUDPServer.address();
            const port = address.port;
            const family = address.family;
            const ipaddr = address.address;
            me.debug('Sevice Scan UDP server port' + port);
            me.debug('Sevice Scan UDP server ip :' + ipaddr);
            me.debug('Sevice Scan UDP server is IP4/IP6 : ' + family);
        });
          
        me.localUDPServer.on('close',function(){
            me.debug('Sevice Scan UDP server closed !');
            me.localUDPServer = null;
        });
          
        me.localUDPServer.bind(me.deviceScanPort);
    }
    //
    //
    //
    ...
    StartMDNSAdvertisement() {
        ...
        const port = this._httpLocalPort > 0 ? this._httpLocalPort : REDserver.address().port;
