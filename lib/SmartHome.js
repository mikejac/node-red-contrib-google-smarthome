    ...
    Start(REDapp, REDserver) {
        try {
            ...
            if (this._httpsPort > 0) {
                ...
                this.dnssdAd = dnssd.Advertisement(dnssd.ServiceType({name: '_nodered-google', protocol: '_tcp', subtypes: []}), this._httpsPort, {txt: {clientId: this._nodeId}}))
                this.dnssdAd.start();
                ...
                if (this._httpNodeRoot !== false) {
                    ...
                    this.dnssdAd = dnssd.Advertisement(dnssd.ServiceType({name: '_nodered-google', protocol: '_tcp', subtypes: []}), REDserver.address().port, {txt: {clientId: this._nodeId}}))
                    this.dnssdAd.start();
                    this._dnssdAdRunning = true;
