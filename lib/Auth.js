...
class Auth {
    ...
    isValidRefreshToken(refreshToken) {
        ...
        if (typeof refreshTokenInfo === 'undefined') {
            this.debug('Auth:isValidRefreshToken(): refreshToken not found ' + JSON.stringify(refreshToken));
            return false;
        }
    ...
    _migrateToMultiUserFormat(user) {
        // Prior to v0.0.XX auth data was saved in another format. Migrate to multi user format if needed.
