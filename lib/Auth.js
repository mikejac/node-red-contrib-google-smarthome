...
class Auth {
    ...
    exchangeAuthCode(code, redirect_uri) {
        let authCode = this._authCode.get(code);

        if (typeof authCode === 'undefined') {
            throw 'invalid code ' + code;
        }

        const expirationDate = new Date(authCode.expiresAt);
        if (expirationDate < Date.now()) {
            throw 'expired code; authCode = ' + JSON.stringify(authCode) + 'expired at ' + expirationDate.toLocaleString();
        }
