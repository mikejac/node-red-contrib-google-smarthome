...
class Auth {
    ...
    renewAuthAccessToken() {
        ...
        try {
            ...
            me.debug('Auth:updatteAuthAccessToken(): new access token expires at ' + new Date(me._auth.expiresAt));
            me._persistAuth();
            me.debug('Auth:updatteAuthAccessToken(): auth data saved = ' + JSON.stringify(me._auth));
            return me._auth.accessToken;
        }
        catch (err) {
            me.debug('Auth:updatteAuthAccessToken(): err: ' + err);
            process.nextTick(() => {
                me.emitter.emit('auth', 'error', err);
            });
        }
    ...
    setJwtKey(jwtkey, userDir) {
        ...
        // this.debug("Auth:setJwtKey(): json " + JSON.stringify(this._jwtkey));
        // this.debug("Auth:setJwtKey(): json project_id " + this.getProjectId());
    }
    //
    //
    //
    ...
    exchangeAccessCode(code) {
        if (!this._auth) {
            throw 'could not find auth info';
        }
        ...
        if (new Date(this._auth.expiresAt) < Date.now()) {
            throw 'expired accessCode' + JSON.stringify(authCode);
        }
