...
class BaseDevice {
    constructor(config, RED) {
        ...
        this.clientConn = this.RED.nodes.getNode(this.client);
        this._debug(".constructor config " + JSON.stringify(config));

        if (!this.clientConn) {
            this.error(this.RED._("device.errors.missing-config"));
            this.status({ fill: "red", shape: "dot", text: "Missing config" });
            return;
        } else if (typeof this.clientConn.register !== 'function') {
            this.error(this.RED._("device.errors.missing-bridge"));
    ...
    getDefaultName(device_type) {
        return this.RED._('device.device_type.' + device_type);
    }
    ...
    parseJson(text, json_text, defaultValue) {
        ...
        if (json_text.trim().length > 0) {
            try {
                ...
                this.RED.log.error('Error on parsing ' + text + ': ' + err.toString());
            }
        }
        return defaultValue;
    }
    ...
    loadJson(text, filename, defaultValue) {
        if (filename) {
            ...
            try {
                ...
                this.RED.log.error('Error on loading ' + text + ' filename ' + filename + ': ' + err.toString());
                return defaultValue;
            }
        }
    }
    ...
    writeJson(text, filename, value) {
        if (filename) {
            ...
            try {
                ...
                this.RED.log.error('Error on saving ' + text + ' filename + ' + filename + ': ' + err.toString());
                return false;
            }
        }
    }
